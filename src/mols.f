...
c	stop
c-------------------------------------------
313     if(isopt.eq.2)then
        do ijx=1,ns
	call output(1,ijx,et,iopt,fseq,aii)
cr        write(*,*)'generated MOLS optimal Structure NO : ',ijx,et
cr	write(31,*)'generated MOLS optimal Structure NO : ',ijx,et
	enddo
	endif
c-------------------------------------------
	return
...
110     if(isopt.eq.2)then
	open(unit=23,file='mols_q.out',status='old')
        do k1=ijx,ijx
        read(23,*) ii,(p_out(k2,i),k2=1,nn)
cs      write(*,*) ii,ij,(p_out(k2,i),k2=1,nn)
	enddo
	endif

cs      print *,'nn-check',nn
cs	STOP
...
C           write(24,10) ijx,e_out(i),(pp(j),j=1,nn)
           write(24,18) ijx,e_out(i),(pp(j),j=1,nn)
...
c	write(*,*)(pp(j),j=1,nn),e_out(i)
	...
	IF(isopt.eq.2)then
	do k1=1,nn
        opmo(ijx,k1) = p_out(k1,i) + 180.0
        pp(k1) = opmo(ijx,k1)
        enddo
        e_out(i) = 0.0
        emo(ijx) = e_out(i)
        do k1 = 1,nn
        p_out(k1,i) = p_out(k1,i)
	enddo
	call molgen3(pp)
	e_out(i) = ampene(ijx,ijx)
	emo(ijx) = e_out(i)
	ELSE IF(isopt.eq.1)then
	do k1=1,nn
...
c18      format(i4,1x,f8.3,<nn>(1x,f6.1))
18      format(i4,1x,<nn>(1x,f6.1))
