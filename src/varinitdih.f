...
c     contact : n_gautham@hotmail.com  
	...
	subroutine varinitdih
	include 'mols.par'
	common /par/ natom, ntor, nhb, ns, lres
	common /fnam/ if1,pf1,pf2,pf3,pf4,of1,of2,of3,of4,op3
	common /tor/ u0(maxpar),sn(maxpar),tn(maxpar),phi(maxpar)
        common /atm/ iatmno(maxatm),atname(maxatm),
     &  rename(maxatm),iresno(maxatm)
	...
	common/xatm/xm(maxstr,maxatm,3)

        dimension x_one(maxatm,3),x_two(maxatm,3)
  	external dihedr
  	integer iff
	character*128 if1,pf1,pf2,pf3,pf4,of1,of2,of3,of4,op3
        
	character*30 atom*30, ele*24
	character*80 str
	character*30 str1
	character*3 str2
	character*6 str3,natoms
...
13	format(a3)
14	format(a6)
115     format(i4,1x,f8.3,1x,<n>(1x,f8.2))
c

	
cr	open(unit=1,file=pf1,status='old')
cr	do i=1,natom
cr		read(1,10) iatmno(i),atname(i),rename(i),iresno(i)
...
c        natom = 75
        open(unit=121,file='sim.log',status='unknown')
                read(121,*)natoms,natom
        close(unit=121)
	open(unit=1,file=pf3,status='old')
	do i=1,ns
	read(1,11)str
	do j=1,natom
	read(1,12)str1,xm(i,j,1),xm(i,j,2),xm(i,j,3)
cr	write(*,*)i,j,xm(i,j,1),xm(i,j,2),xm(i,j,3)
	enddo
	read(1,13)str2
	read(1,14)str3
	enddo
        ...
        open(unit=11,file=op3,status='old')
 
        do i = 1,natom
          read(11,10) iatmno(i),atname(i),rename(i),iresno(i)
cr	  write(*,10) iatmno(i),atname(i),rename(i),iresno(i)
        enddo
        close(unit=11)
...
c----------measure the dihedral angles of initial model -----------------
	...
	dih = 0.0
	  ...
	  i3 = ivat(i,3)
	  i4 = ivat(i,4)
cc        write(*,*)ivat(i,1),ivat(i,2),ivat(i,3),ivat(i,4)
        if(i1.ne.0.and.i2.ne.0.and.i3.ne.0.and.i3.ne.0) then
	dih = dihedr(j,i1,i2,i3,i4)
	endif
	phi(i) = dih
c	print *,j,i,i1,i2,i3,i4,dih
	enddo
c
...
c********************************************************************
	...
	character*128 if1,pf1,pf2,pf3,pf4,of1,of2,of3,of4,of0,op3
	character*30 atom
        character*24 ele
...
c*******************************************************************
c************************************************************************	
       function dihedr(im,i1,i2,i3,i4)
	include 'mols.par'
	common /par/ natom, ntor, nhb, ns, lres
	common/xatm/xm(maxstr,maxatm,3)
        common/crda/x(maxatm,8)
