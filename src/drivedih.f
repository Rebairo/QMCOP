...
c***********Out put generate file names**********************
	...
	op3 = path(:LNBLNK(path)) // '/' // 
     &        mname(:LNBLNK(mname)) // '_' // 'inpp.pdb'
        print *,op3
c	print *,if1,of0,of1,pf1,pf2,pf3,pf4,of1,of2,of3,of4
c************************************************************
cr	open(unit=31,file=of0,status='unknown')
cr      WRITE (31,1)
cr      WRITE (*,1)
cr1     FORMAT (//
cr     *10X,'#########################################################'/
...
c
cr	write(31,2) '**********CLUSTERING MINIMIZED STRUCTURES**************'
cr	write(*,2) '**********CLUSTERING MINIMIZED STRUCTURES**************'
cr	if(ico.eq.1) call cluster(icnt)
cr	if(ico.eq.2) call mclust(icnt,rk1,kr2)
c
cr	print *,'cluster o.k'
c
c*******Flushing standard output************
cr	call flush(6)
c*******End of Flushing ********************
...
cr919      CALL DATE(DAY)
cr      CALL TIME(HOUR)
cr      WRITE (31,'(2(/25X,2A/))') 'Job ended on ', DAY, 
cr     &                               '      at ', HOUR
cr      WRITE (*,'(2(/25X,2A/))') 'Job ended on ', DAY,
cr     &                               '      at ', HOUR
cr	write (*,*)'cpu time = : ',etime(tt)
cr	write(31,*)'cpu time = : ',etime(tt)
c*******Flushing standard output************
cr	call flush(6)
c*******End of Flushing ********************
