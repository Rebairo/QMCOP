...
cr	if(lres.gt.15) then
cr	 write(31,*) 'you exceeded the upper limit of sequence
cr    & length, so you need to calculate the initial global cluter cutoff
cr     & value by uncommending the lines in the cluster routine or yourself
cr     & using non-homologus protein chains'
cr	write(31,*)'do you want to continue?(y or n)'
cr	write(31,*) 'if yes program will take the default value'
cr	write(31,*) 'if no you have to supply the global cutoff value'
cr	read(5,56) yn
cr	if(yn.eq.'n') stop
cr	if(yn.eq.'y') gcut = pgcut(15)
cr	endif
...
c	print *,i,j,r,rr
	...
	do i = 1,ni1
	do j = i+1,ni2
	std = std + (rmsd(i,j) - avrg)**2
	enddo
	enddo
	sigma = sqrt(std/n)
	cgcut = avrg - sigma
	write(31,*) avrg,'sigma =',sigma,cgcut,n
c
c	set default cluster cutoff: whichever is small cgcut or rgcut?
c
cr	if(rgcut.lt.cgcut) then
cr	gcut = rgcut
cr	else 
	gcut = cgcut
cr	endif
